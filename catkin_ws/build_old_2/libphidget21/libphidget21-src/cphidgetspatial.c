...
CPHIDGETINIT(Spatial)
	...
	switch(phid->phid.deviceUID)
	{
		...
		case PHIDUID_SPATIAL_ACCEL_GYRO_COMPASS_1044_1:
			phid->accelerationMax = 8.1;
			phid->accelerationMin = -8.1;
			phid->interruptRate = 4;
			phid->dataRateMin = SPATIAL_MIN_DATA_RATE;
			...
			phid->angularRateMax = 2000.1;
			phid->angularRateMin = -2000.1;
			phid->magneticFieldMax = 50.1;
			phid->magneticFieldMin = -50.1;
			phid->userMagField = 1.0;
			phid->calDataValid = PFALSE;
			break;
...
static float
unpackfloat(const unsigned char *buffer) {
	float ret;
	unsigned int *retIntPtr = (unsigned int*)& ret;

	// Incoming data is 4-byte little endian
	// Do things this way, so that the data ends up in the target 32-bits in host machine byte order
	*retIntPtr = (unsigned int)((unsigned int)buffer[3] << 24)
...
//Zero out calibrations
		...
		case PHIDUID_SPATIAL_ACCEL_GYRO_COMPASS_1044_1:
		{
			...
			//this timestamp is for the latest data
			time = (unsigned int)((unsigned int)buffer[4] << 24)
	...
	double gainPositive[3], double gainNegative[3], double offset[3], double factor1[3], double factor2[3])
{
	...
	switch(phid->phid.deviceUID)
	{
		...
		case PHIDUID_SPATIAL_ACCEL_GYRO_COMPASS_1044_1:
			if (index == SPATIAL_ANALOG_GYRO_CALIB_TABLE_INDEX) {
				offsetMultipliers[0] = offsetMultipliers[1] = SPATIAL_ADXRS290_BITS_PER_DPS;
				offsetMultipliers[2] = SPATIAL_ADXRS810_BITS_PER_DPS;
				break;
			}
			if (index == SPATIAL_ANALOG_ACCEL_CALIB_TABLE_INDEX) {
				offsetMultipliers[0] = offsetMultipliers[1] = offsetMultipliers[2] = SPATIAL_LIS3DHH_BITS_PER_G;
				break;
			}
			if (index == SPATIAL_DIGITAL_GYRO_CALIB_TABLE_INDEX) {
				offsetMultipliers[0] = offsetMultipliers[1] = offsetMultipliers[2] = SPATIAL_L3GD20_BITS_PER_DPS;
			...
			if (index == SPATIAL_DIGITAL_ACCEL_CALIB_TABLE_INDEX) {
				offsetMultipliers[0] = offsetMultipliers[1] = offsetMultipliers[2] = SPATIAL_LIS2HH12_BITS_PER_G;
				break;
			} else
				return EPHIDGET_UNSUPPORTED;
