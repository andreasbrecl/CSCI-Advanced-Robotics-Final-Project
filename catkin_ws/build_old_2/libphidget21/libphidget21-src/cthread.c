...
int CThread_wait_on_event(EVENT *ev, EVENT_TIME ms) {
#ifdef _WINDOWS
	...
	if (ev->ready_to_go == PFALSE) {
		if (ms == INFINITE)
			...
			psec = tv.tv_usec * 1000ULL + ms * 1000000;
...
#endif
			retval = pthread_cond_timedwait(&ev->condition, &ev->mutex, &ts);
		}
