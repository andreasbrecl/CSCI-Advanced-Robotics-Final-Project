...
CPHIDGETINIT(AdvancedServo)
	...
	switch(phid->phid.deviceIDSpec)
	{
		...
		case PHIDID_ADVANCEDSERVO_1MOTOR:
			if ((phid->phid.deviceVersion >= 100) && (phid->phid.deviceVersion < 500))
			{
...
CPHIDGETDATA(AdvancedServo)
	...
	switch(phid->phid.deviceIDSpec)
	{
		case PHIDID_ADVANCEDSERVO_8MOTOR:
			...
			//Voltage control, no current sense
				...
				for (i = 0; i < phid->phid.attr.advancedservo.numMotors; i++) {
					...
					speedRamping[i] = (buffer[0 + (i * 5)] & NO_RAMPING_FLAG_ADVSERVO) ? PFALSE : PTRUE;
					motorDone[i] = (buffer[0 + (i * 5)] & MOTOR_DONE_ADVSERVO) ? PTRUE : PFALSE;

					pwmEcho[i] = (unsigned short)(buffer[1 + (i * 5)] << 8) + (unsigned char)(buffer[2 + (i * 5)]);
					position[i] = pwmEcho[i] / 12.0;

					velocity[i] = (signed short)(buffer[3 + (i * 5)] << 8) + (unsigned char)(buffer[4 + (i * 5)]);
					velocity[i] = round_double(((velocity[i] / 16384.0)*phid->velocityMaxLimit), 2);
