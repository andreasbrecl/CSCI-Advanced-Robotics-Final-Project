...
days_from_civil(GPSDate date) {
	...
	era = (date.tm_year >= 0 ? date.tm_year : date.tm_year - 399) / 400;
	yoe = date.tm_year - era * 400;      // [0, 399]
	doy = (153 * (date.tm_mon + (date.tm_mon > 2 ? -3 : 9)) + 2) / 5 + date.tm_mday - 1;  // [0, 365]
...
static int parse_NMEA_data(char *data, CPhidgetGPSInfo *phid) {
	...
	if(!strncmp("GGA",dataarray[0]+3,3)) {
		...
		if(strlen(dataarray[9])>=6) {
			...
			date.tm_mday = (short)(intpart / 10000);
			date.tm_mon = (short)(intpart / 100 % 100);
			date.tm_year = (short)(intpart % 100) + 2000; //2-digit year, add 2000 years
			...
			phid->GPSData.RMC.date = civil_from_days(days);
			phid->haveDate = PTRUE;
		}
		else
