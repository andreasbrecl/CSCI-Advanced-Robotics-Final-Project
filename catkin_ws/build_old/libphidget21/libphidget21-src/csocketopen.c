...
int setupHeartbeat(CPhidgetSocketClientHandle server, pdc_listen_id_t *id) {
	...
	escape(addr, (unsigned int)strlen(addr), &a);
...
int setupKeysAndListeners_phidget(CPhidgetHandle phid, pdc_listen_id_t *id) {
	...
	TESTPTR(phid)
		...
		if (phid->specificDevice == PHIDGETOPEN_SERIAL) {
			...
			char *l;
			escape2(phid->label, (unsigned int)strlen(phid->label), &l, PTRUE); //make sure the backslashes are escaped because this is a regex
		...
		if (phid->specificDevice == PHIDGETOPEN_SERIAL) {
			...
			char *l;
			escape(phid->label, (unsigned int)strlen(phid->label), &l);
