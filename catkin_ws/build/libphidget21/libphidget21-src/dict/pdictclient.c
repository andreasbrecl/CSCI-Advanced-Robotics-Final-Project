...
jgetresult_cb(pdc_session_t *pdcs, result_t *r, int code, int final,
	...
	int spaceleft = grcba->grcba_reply_len - (int)(grcba->grcba_reply_ptr - grcba->grcba_reply);
	...
	if ((size_t)spaceleft < strlen(line) + 1) {
		int offset = (int)(grcba->grcba_reply_ptr - grcba->grcba_reply);
		if (!(grcba->grcba_reply = (char *)realloc(grcba->grcba_reply, strlen(line) + offset + 100))) {
			LOG(PHIDGET_LOG_WARNING, "Couldn't realloc!");
			return;
		}
		grcba->grcba_reply_len = (int)strlen(line) + offset + 100;
		grcba->grcba_reply_ptr = grcba->grcba_reply + offset;
		spaceleft = grcba->grcba_reply_len - (int)(grcba->grcba_reply_ptr - grcba->grcba_reply);
	}
